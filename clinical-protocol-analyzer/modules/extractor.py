...
def extract_trial_entities(llm: LLMBase, chunks: List[dict], strict: bool = True) -> Dict[str, str]:
    ...
    def clean_term(term: str) -> str:
        """
        Clean extracted condition/intervention for search queries:
        - Remove text inside parentheses
        - Take only the first part before slash or comma
        - Collapse multiple spaces
        """
        term = re.sub(r"\([^)]*\)", "", term)  # Remove parentheticals
        term = re.split(r"[/,]", term)[0]      # First part before / or ,
        term = re.sub(r"\s+", " ", term).strip()  # Normalize spaces
        return term

    keys = ["indication", "disease", "condition", "intervention",
            "investigational product", "drug", "therapy", "biologic", "device"]

    candidates = [c for c in chunks if any(k in c["text"].lower() for k in keys)]
    contexts = [c["text"][:3000] for c in candidates[:20]]

    q = (
        "Extract the clinical CONDITION/INDICATION and INTERVENTION (drug/device/biologic/procedure) "
        "named in the protocol. Return strict JSON with keys: "
        "condition (string), intervention (string), aliases (array of strings for condition synonyms/abbreviations). "
